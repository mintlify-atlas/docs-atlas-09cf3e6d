...
<Note>
  The default value for `bc_voice_chat_server_address` is `"managed"`, which tells BestClient to resolve the relay endpoint from BestClient-run infrastructure automatically. You only need to change this if you are self-hosting a compatible voice relay or were given an alternate address by the BestClient team.
</Note>
